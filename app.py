...
@app.route('/checksymptoms', methods=['GET'])
def checksymptoms():
...
def add_login():
    ...
    val = ()
    for field in fields:
        temp = request.form[field]
        val += ("\'"+temp+"\'", )
        print(temp)

    temp = password_check(val[1])
    print(temp, 'rakesh')
    if (temp == 'valid'):
        print("Password is valid")
    else:
        return render_template('register.html', validation = temp)

    if val[1]!=val[2]:
      return render_template('register.html', validation = 'verify password correctly')
    print(val)
    qry = "INSERT INTO login Values (%s,%s)"%val[:-1]
    print(qry)
    success = True
    error = False
    ...
    try:
        mycursor.execute(qry)
        # validation = ''
        # validation = 'user registered, go to Login page'
        return render_template('loginuser.html')
        ...
        error = True
        success = False
        validation = 'user not registered , Try again'
